...
function generateIndex(outputDir, opts) {
  ...
  lines.push("");
  lines.push("## Sections");
  lines.push("## entry     — entry point functions (no callers, start analysis here)");
  lines.push("## alerts    — security patterns detected (URL, eval, fingerprint, etc.)");
  ...
  lines.push("## fn/<cat>  — functions grouped by category (see CATEGORY_LABELS)");
  lines.push("");
  lines.push("## Function Entry Format");
  lines.push("name | Ss/Pp | cc=N | → callees ⇐ callers | closure: v1, v2 ; paramRoles ; [semTags] ; flags");
  lines.push("");
  lines.push("## Fields");
  ...
  lines.push("closure: v  = captures variable v from outer scope (closure capture)");
  lines.push("");
  ...
  lines.push(";  = extras separator (between closure, paramRoles, semTags, and flags)");
  lines.push("");
